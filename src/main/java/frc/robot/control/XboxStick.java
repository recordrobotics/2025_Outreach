...
public class XboxStick extends AbstractControl {
  ...
  public DriveCommandData getDriveCommandData() {
    ...
    DriveCommandData driveCommandData =
        new DriveCommandData(
            -(getXY().getFirst() + getAB().getFirst()) * getDirectionalSpeedLevel(),
            (getXY().getSecond() + getAB().getSecond()) * getDirectionalSpeedLevel(),
  ...
  public Pair<Double, Double> getXY() {
    double X =
        SimpleMath.ApplyThresholdAndSensitivity(
            drivebox.getRawAxis(0), Constants.Control.XBOX_X_THRESHOLD, 1);
    double Y =
        SimpleMath.ApplyThresholdAndSensitivity(
            drivebox.getRawAxis(1), Constants.Control.XBOX_X_THRESHOLD, 1);
